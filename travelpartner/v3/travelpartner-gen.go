...
const basePathTemplate = "https://travelpartner.UNIVERSE_DOMAIN/"
...
// AccountLink: An account link. Represents the link between a Google Ads
// customer and a Hotel Ads (Hotel Center) account. An account link defines the
// set of hotels under the Hotel Center account that is linked to the Google
// Ads customer.
type AccountLink struct {
	// AccountLinkTarget: Required for CREATE requests. The properties in the Hotel
	// Center account that can be managed using the linked Google Ads account.
	AccountLinkTarget *AccountLinkTarget `json:"accountLinkTarget,omitempty"`
	// GoogleAdsCustomerName: Required for CREATE requests. The value representing
	// the Google Ads customer ID in the format
	// `customers/{google_ads_customer_id}`. For example: `customers/0123456789`.
	// Note that the `googleAdsCustomerName` field is not returned in responses to
	// GET requests.
	...
	//   "ACCOUNT_LINK_STATUS_UNKNOWN" - The account link status is not recognized
	// by this API version.
	//   "REQUESTED_FROM_HOTEL_CENTER" - The account link was initiated through
	// Hotel Center and has not yet been approved in Google Ads.
	//   "REQUESTED_FROM_GOOGLE_ADS" - The account link was initiated through
	// Google Ads and has not yet been approved in Hotel Center.
	//   "APPROVED" - The account link has been approved.
	Status string `json:"status,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "AccountLinkTarget") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AccountLinkTarget") to include in
...
// AccountLinkTarget: Defines whether all properties or a subset of properties
// in the Hotel Center account can be managed with the linked Google Ads
// account. If a subset, the specific properties are specified.
type AccountLinkTarget struct {
	// AllHotels: True if all properties under the Hotel Center account are made
	// available to the account link. The default is `false`.
	AllHotels bool `json:"allHotels,omitempty"`
	// HotelList: List of properties that is made available to the account link.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllHotels") to include in API
...
// Brand: Brand-level icon and display name configuration. Once approved, the
// icon and display name appear in the search results for properties that the
// partner has assigned to this brand.
type Brand struct {
	// ActiveDisplayNames: Output only. The brand's active display names in all
	// provided languages, only present if the display name is approved for all
	// provided languages.
	...
	// (/hotels/hotel-prices/api-reference/rest/v3/accounts.icons#resource:-iconreso
	// urce). The value refers to the icons's resource name in the format
	// `accounts/{account_id}/icons/{icon_id}`. An active icon is one that has been
	// approved.
	ActiveIcon string `json:"activeIcon,omitempty"`
	// ActiveIconUri: Output only. URL of the active icon, only present when the
	...
	// reason. Only applies to display names with the review state 'REJECTED'.
	DisplayNameDisapprovalReason []*DisplayNameDisapprovalReason `json:"displayNameDisapprovalReason,omitempty"`
	// DisplayNameState: Output only. The brand's display names review state, which
	// applies to all display name language entries. If there are both submitted
	// and active display names, this refers to the submitted display names.
	//
	// Possible values:
	//   "REVIEW_STATE_UNSPECIFIED" - Default value.
	//   "REVIEW_STATE_NEW" - Google is reviewing the icon or display name.
	//   "APPROVED" - The icon or display name is approved and ready for public
	// display.
	//   "REJECTED" - The icon or display name is rejected. Google won't display
	// it.
	DisplayNameState string `json:"displayNameState,omitempty"`
	// DisplayNames: Input only. The name Google displays for the brand's
	// properties. Setting the display names is only necessary if you want to
	// override the landing page display name or account-level display name for the
	// brand. Google reviews the display names for appropriate content. When there
	// are multiple languages, Google will only show the display names once Google
	// approves all the languages.
	DisplayNames []*LocalizedText `json:"displayNames,omitempty"`
	// Icon: Input only. The brand's accounts.icon
	// (/hotels/hotel-prices/api-reference/rest/v3/accounts.icons#resource:-iconreso
	// urce) identifying the brand's icon. The value refers to the icons's resource
	// name in the format `accounts/{account_id}/icons/{icon_id}`.
	Icon string `json:"icon,omitempty"`
	// IconDisapprovalReasons: Output only. The icon's disapproval reason(s). Only
	// applies to submitted icons with `REJECTED` state.
	//
	// Possible values:
	//   "IMAGE_DISAPPROVAL_REASON_UNSPECIFIED" - Not specified.
	//   "NOT_LIKE_SITE" - Icon image does not match the favicon or artwork on the
	// advertiser’s final URL.
	//   "OFFENSIVE" - Icon image contains offensive content.
	//   "LOW_QUALITY" - Icon image is low quality.
	//   "ANIMATED" - Icon image is animated.
	//   "BAD_BACKGROUND" - Icon image does not have a transparent or non-white
	// background.
	...
	// IconState: Output only. The brand's icon's review state. If there are both
	// submitted and active icons, this refers to the submitted icon.
	//
	// Possible values:
	//   "REVIEW_STATE_UNSPECIFIED" - Default value.
	//   "REVIEW_STATE_NEW" - Google is reviewing the icon or display name.
	...
	// `accounts/{account_id}/brands/{brand_id}`. The `brand_id` corresponds to the
	// partner's brand identifier used for landing page matching and the
	// property-level brand identifier. A default brand is applied to properties
	// that do not have a brand. The `brand_id` of the default brand is
	// `NO_BRAND_ID`. It can be fetched and updated like any configured brand.
	Name string `json:"name,omitempty"`
	// PropertyCount: Output only. The number of properties with the corresponding
	// brand ID.
	PropertyCount int64 `json:"propertyCount,omitempty,string"`
	// SubmittedDisplayNames: Output only. The brand's submitted display names in
	// all provided languages, only present if the display name is new or rejected
	// for any language.
	...
	// (/hotels/hotel-prices/api-reference/rest/v3/accounts.icons#resource:-iconreso
	// urce). The value refers to the icons's resource name in the format
	// `accounts/{account_id}/icons/{icon_id}`. A submitted icon is one that is new
	// or rejected.
	SubmittedIcon string `json:"submittedIcon,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "ActiveDisplayNames") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ActiveDisplayNames") to include
...
type CreateReconciliationReportResponse struct {
	...
	// ReconciliationReport: Resource name of the new reconciliation report. The
...
type DataIssueDetail struct {
	...
	//   "WARNING" - The listing will still be eligible for serving, but the data
	// issue may have an adverse effect on the content shown.
	...
	//   "DUPLICATE_ADDRESS" - The address is the same as another property in the
	// Hotel List Feed.
	//   "MISSING_PHYSICAL_STREET_ADDRESS" - The provided address appears to
	// contain a postal box instead of a physical street address.
	//   "MISSING_STREET_NAME" - The address does not appear to contain a street
	// name.
	//   "MISSING_STREET_NUMBER" - The address does not appear to contain a street
	// number.
	...
	//   "INVALID_POSTAL_CODE_SUFFIX" - The address contains an invalid postal code
	// suffix.
	//   "UNEXPECTED_POSTAL_CODE_SUFFIX" - The address contains a postal code
	// suffix when unsupported by the country's postal code system.
	...
	// 'listing.content.attributes.attr' is either unsupported or has an invalid
	// value.
	//   "INVALID_EMAIL_ADDRESS" - The email at
	// 'listing.content.attributes.attr[name='email']' is not a valid email
	// address.
	//   "DUPLICATE_LATLONG" - The latitude/longitude pair is the same as another
	// property in the Hotel List Feed.
	//   "LATLONG_INCONSISTENT_WITH_ADDRESS" - The latitude/longitude pair provided
	// for the address is more than 500 meters away from the Google
	// latitude/longitude generated from the address.
	//   "MISSING_LATLONG" - A valid pair of latitude/longitude fields is missing
	// and Google latitude/longitude generation is disabled.
	//   "COULD_NOT_GEOCODE" - A valid pair of latitude/longitude fields is missing
	// and Google could not generate them from the address.
	//   "MISSING_HOTEL_NAME" - No hotel name is provided.
	//   "HOTEL_NAME_EMPTY" - An empty or whitespace only name is provided, but no
	// valid name.
	...
	//   "PARSE_ERROR_IN_XML" - Google encountered an error while importing your
	// Hotel List Feed.
	//   "UNEXPECTED_ATTRIBUTE_IN_XML" - Google encountered an unknown error while
	// importing your Hotel List Feed.
	//   "DUPLICATE_PHONE_NUMBER" - The phone number you provided for this property
	// is the same as the phone number for another property. More than one listing
	// in the Hotel List Feed cannot have the same phone number.
	...
	//   "INVALID_PHONE_NUMBER_COUNTRY_CODE" - The country code of the phone number
	// is not valid.
	...
	// `content.attributes.client_attr[name='hotel_brand']` element exceeds 70
	// Unicode characters, and/or `content.attributes.client_attr[name='custom_*']`
	// exceeds 100 Unicode characters.
	//   "FLAGGED_DUE_TO_SUSPICIOUS_METADATA" - Contact your account manager for
	// details.
	//   "NOT_ENOUGH_IMAGES_PROVIDED" - Vacation rental properties must have at
	// least five feed images. Eight or more images are strongly recommended;
	// having fewer than 8 may impact performance (such as impressions and clicks).
	//   "IMAGE_PROCESSING_IN_PROGRESS" - The images for this vacation rental
	// property are still being processed.
	//   "CANNOT_FETCH_IMAGES" - The images for this property cannot be fetched.
	// Please check that the image URLs are valid (e.g. do not give a 404 error)
	// and that the domain hosting the image URLs does not block crawling of these
	// images in its robots.txt file or X-Robots-Tag HTTP Header.
	//   "INCOMPATIBLE_IMAGE_SIZE_OR_LOW_QUALITY" - The images provided for this
	// property do not meet Google's requirements due to dimensions, size, or
	// automated quality signals. Please ensure that the images are less than 10 MB
	// large, less than 4K pixels wide, and less than 4K pixels tall. Please check
	// additional guidance outlined at the [Property listing image
	// requirements](/hotels/vacation-rentals/dev-guide/onboarding#property_listing_
	// image_requirements)
	//   "MISSING_LANG_IN_RAW_LISTING" - Deprecated Issue Vacation rental raw
	// listing is missing a language field.
	//   "IS_HOTEL" - Vacation rental listing has been characterized as a hotel and
	...
	//   "NAME_AND_DESCRIPTION_ABUSE_VERIFIED" - Vacation rental listing's name or
	// description is verfied to contain content that violates Google's content
	// policy.
	...
	//   "NON_VR_ACCOMMODATION_TYPE" - Listing that is sent as a vacation rental is
	...
	// such as the country in both addr1 and country_code. 2. Use Google Maps to
	// confirm if the address is formatted correctly for its region. 3. Use a [plus
	// code](https://plus.codes) if the address continues to be invalid.
	//   "VR_ADDRESS_INCOMPLETE" - Deprecated: Replaced with more specifc errors or
	// VR_ADDRESS_INVALID. Google detected that the provided address for this
	// listing is incomplete. For example, it may be missing a unit number.
	//   "VR_ADDRESS_INCOMPLETE_NO_STREET_NAME" - The provided address is missing a
	// street name.
	//   "VR_ADDRESS_INCOMPLETE_NO_STREET_NUMBER" - The provided address is missing
	// a street number.
	//   "VR_ADDRESS_INCOMPLETE_NO_UNIT_NUMBER" - The provided address is missing a
	// unit number.
	//   "VR_ADDRESS_LATLONG_MISMATCH" - The provided address and lat/long data for
	// this listing doesn't match.
	//   "INVALID_REVIEW_RATING" - Vacation rental listing has an invalid review
	// rating.
	//   "INVALID_CHECKIN_FORMAT" - Vacation rental listing has an invalid check-in
	// format.
	//   "INVALID_CHECKOUT_FORMAT" - Vacation rental listing has an invalid
	// check-out format.
	//   "INTERNAL_BLOCKLIST" - This listing is in an internal blocklist. Contact
	// your account manager for help.
	//   "VR_LIVE_ON_GOOGLE_INACTIVE_ERROR" - The "Live on Google" setting for this
	// Vacation Rental is set to false. Note: once "Live on Google" is set to true,
	// it will take a few days for this error to disappear.
	//   "VR_XML_FAILED_PARSING" - This listing's XML failed to parse. Please fix
	// the XML associated with this listing.
	//   "VR_ZIP_FILE_CORRUPT" - A listing that indicates the that feed ZIP file is
	// corrupt. Please ensure the ZIP file is valid.
	//   "VR_PENDING_INITIAL_PROCESSING" - Vacation rental listing is undergoing
	// initial processing.
	//   "VR_WEBSITE_LINK_ROBOTED" - Vacation rental listing has a website link
	// that is blocked by a robots.txt
	//   "VR_PENDING_WEBSITE_CRAWLING" - Vacation rental listing has a website link
	// that is still undergoing crawling.
	//   "VR_WEBSITE_LINK_WITH_CRAWLING_ERROR" - Vacation rental listing has a
	// website link with a crawling error.
	...
	// to resolve the issue. Refer to documentation on the data issue’s type for
	// further information.
	IsSelfResolving bool `json:"isSelfResolving,omitempty"`
	// ForceSendFields is a list of field names (e.g. "DataIssueSeverity") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DataIssueSeverity") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// DisplayNameDisapprovalReason: Disapproval reason of the display name in a
// specific language.
type DisplayNameDisapprovalReason struct {
	...
	//   "LANDING_PAGE_NOT_MATCHED" - The display name does not match the landing
	// page.
	...
	// ForceSendFields is a list of field names (e.g. "DisapprovalReason") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DisapprovalReason") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// FreeBookingLinksResult: **DEPRECATED:** Use `PropertyPerformanceResult` with
// `PropertyPerformanceReportService` instead. Represents a result from
// querying for free booking link stats for an account.
type FreeBookingLinksResult struct {
	// ClickCount: The total number of clicks that were recorded for this result.
	ClickCount int64 `json:"clickCount,omitempty,string"`
	// Date: A date within the requested date range. Only present if `date` is
	...
	// `partnerHotelDisplayName` is specified in `aggregateBy` in the request.
	PartnerHotelDisplayName string `json:"partnerHotelDisplayName,omitempty"`
	// PartnerHotelId: Partner's hotel ID. Only present if `partnerHotelId` is
	// specified in `aggregateBy` in the request.
	PartnerHotelId string `json:"partnerHotelId,omitempty"`
	// UserRegionCode: ISO 3116 region code of the country/region of the user. Only
	// present if `userRegionCode` is specified in `aggregateBy` in the request
...
// HotelList: The properties in the Hotel Center account that can be managed
...
type HotelList struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PartnerHotelIds") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type HotelPricePerItinerary struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CheckinDate") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type HotelView struct {
	// DataIssueDetail: The type and severity of each data quality issue in the
	// feed. If your feed contains data issues, update the feed and resubmit it.
	// For more information, refer to this article.
	DataIssueDetail []*DataIssueDetail `json:"dataIssueDetail,omitempty"`
	// DataIssues: DEPRECATED. Indicates that the hotel has data quality issues.
	// The value of this field indicates the type of error. This has been replaced
	// with the data_issue_detail field. If your feed contains data issues, update
	// the feed and resubmit it. For more information, refer to this article.
	//
	// Possible values:
	//   "FEED_DATA_ISSUE_UNSPECIFIED" - Not specified.
	//   "FEED_DATA_ISSUE_UNKNOWN" - The value is unknown in this version.
	...
	//   "MAP_OVERLAP" - More than one of your hotels match this hotel on Google
	// Maps.
	MatchStatus string `json:"matchStatus,omitempty"`
	// OverclusteredPartnerHotelIds: Other hotels with which the hotel is
	// overclustered. If your feed contains overclustered hotels, update the feed
	// and resubmit it. To do this, you can use the manual match tool.
	...
	PartnerHotelDisplayName string `json:"partnerHotelDisplayName,omitempty"`
	// PartnerHotelId: The unique ID of the hotel that the partner provides in
	// their Hotel List Feed.
	PartnerHotelId string `json:"partnerHotelId,omitempty"`
	// PrimaryOverclusteredPartnerHotelId: The primary hotel in the overclustered
	// set.
	PrimaryOverclusteredPartnerHotelId string `json:"primaryOverclusteredPartnerHotelId,omitempty"`
	// PropertyDetails: Optional. A URL to the property on Google. Only available
	// for properties that are listed.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DataIssueDetail") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type Icon struct {
	// DisapprovalReasons: Output only. The icon's disapproval reason(s). Only
	// applies to icons with `REJECTED` state.
	//
	// Possible values:
	//   "IMAGE_DISAPPROVAL_REASON_UNSPECIFIED" - Not specified.
	//   "NOT_LIKE_SITE" - Icon image does not match the favicon or artwork on the
	...
	// IconUri: Output only. The approved icon's Google-hosted URI. Only applies to
	// icons with `APPROVED` state.
	IconUri string `json:"iconUri,omitempty"`
	// ImageData: Required. Input only. The icon contents, which must be in PNG
	// format, or convertible to PNG.
	ImageData string `json:"imageData,omitempty"`
	// Name: Required. Output only. The resource name for the icon in the format
	// `accounts/{account_id}/icons/{icon_id}`. Google generates the `icon_id`
	// during the `create` operation. Use the `icon_id` to associate the icon with
	// a brand using the accounts.brands
	// (/hotels/hotel-prices/api-reference/rest/v3/accounts.brands#resource:-brand)
	// API.
	Name string `json:"name,omitempty"`
	// Reference: Optional. Value for tracking the icon. It could be the primary
	// key to your icon in your system, or the icon's filename. Google does not use
	// this value.
	...
	//   "APPROVED" - The icon is approved and is ready for public display. You can
	// link APPROVED icons to brands.
	//   "REJECTED" - The icon is rejected. Google won't display this icon.
	State string `json:"state,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "DisapprovalReasons") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DisapprovalReasons") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Image: A single image in a VR Listing. Title and gallery URL are stored per
// locale. NEXT ID: 4
type Image struct {
	// GalleryUri: URL that is deeplink to the image in a gallery (or a webpage).
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "GalleryUri") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Key: Key of a result.
type Key struct {
	// AdvanceBookingWindow: The number of days in advance the user wants to book
	// the itinerary. If `advanceBookingWindow` is not a value of the `aggregateBy`
	// parameter in the request call, then the `advanceBookingWindow` field is not
	// returned in the `Key`.
	AdvanceBookingWindow int64 `json:"advanceBookingWindow,omitempty"`
	// CheckinDate: The check-in date of the itinerary. If `checkinDate` is not a
	// value of the `aggregateBy` parameter in the request call, then the
	// `checkinDate` field is not returned in the `Key`.
	CheckinDate *Date `json:"checkinDate,omitempty"`
	// Date: The date for which you are requesting metrics. If `date` is not a
	// value of the `aggregateBy` parameter in the request call, then the `date`
	// field is not returned in the `Key`.
	Date *Date `json:"date,omitempty"`
	// DeviceType: The user’s device type. If `deviceType` is not a value of the
	// `aggregateBy` parameter in the request call, then the `deviceType` field is
	// not returned in the `Key`.
	//
	// Possible values:
	//   "DEVICE_UNSPECIFIED" - Not specified.
	//   "DEVICE_UNKNOWN" - The value is unknown in this version.
	...
	//   "TABLET" - Tablets with full browsers.
	DeviceType string `json:"deviceType,omitempty"`
	// HotelRegionCode: CLDR region code of the country/region of the hotel. If
	// `hotelRegionCode` is not a value of the `aggregateBy` parameter in the
	// request call, then the `hotelRegionCode` field is not returned in the `Key`.
	HotelRegionCode string `json:"hotelRegionCode,omitempty"`
	// LengthOfStayDays: The number of nights for the itinerary. If
	// `lengthOfStayDays` is not a value of the `aggregateBy` parameter in the
	// request call, then the `lengthOfStayDays` field is not returned in the
	// `Key`.
	LengthOfStayDays int64 `json:"lengthOfStayDays,omitempty"`
	// Occupancy: The total occupancy of the itinerary. If `occupancy` is not a
	// value of the `aggregateBy` parameter in the request call, then the
	// `occupancy` field is not returned in the `Key`.
	Occupancy int64 `json:"occupancy,omitempty"`
	// PartnerHotelId: Partner's hotel ID. If `partnerHotelId` is not a value of
	// the `aggregateBy` parameter in the request call, then the `partnerHotelId`
	// field is not returned in the `Key`.
	PartnerHotelId string `json:"partnerHotelId,omitempty"`
	// UserRegionCode: ISO 3116 region code of the country/region of the user. If
	// `userRegionCode` is not a value of the `aggregateBy` parameter in the
	// request call, then the `userRegionCode` field is not returned in the `Key`.
	UserRegionCode string `json:"userRegionCode,omitempty"`
	// ForceSendFields is a list of field names (e.g. "AdvanceBookingWindow") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdvanceBookingWindow") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ListAccountLinksResponse struct {
	// AccountLinks: A list of all account links associated with the Hotel Center
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AccountLinks") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ListBrandsResponse struct {
	// Brands: A list of all brands associated with the Hotel Center account being
	// queried.
...
type ListHotelViewsResponse struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "HotelViews") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ListIconsResponse struct {
	// Icons: A list of all icons associated with the queried partner account.
	Icons []*Icon `json:"icons,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "Icons") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
type ListPriceAccuracyViewsResponse struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "PriceAccuracyViews") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PriceAccuracyViews") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ListPriceCoverageViewsResponse struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "PriceCoverageViews") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PriceCoverageViews") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ListReconciliationReportsResponse struct {
	// ReconciliationReports: The list of names of reconciliation reports. Note
	// that the `contents` and `fileName` fields of each `ReconciliationReport`
	// object are not returned by this call.
	ReconciliationReports []*ReconciliationReport `json:"reconciliationReports,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "ReconciliationReports") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// MissedParticipationCountDetails: Missed participation count broken down by
// reason.
type MissedParticipationCountDetails struct {
	// HotelSuspendedCount: The total number of missed participations due to one or
	// more of your hotels being suspended due to price accuracy violations.
	HotelSuspendedCount int64 `json:"hotelSuspendedCount,omitempty,string"`
	// NoAvailabilityCount: The total number of missed participation due to the
	// hotel/itinerary combination being unavailable, or the traveler was
	// ineligible for the rates. To participate in these auctions, you may need to
	// provide more pricing information.
	...
	// NoPriceCount: The total number of missed participations due to a price not
	// being offered for the requested itinerary.
	NoPriceCount int64 `json:"noPriceCount,omitempty,string"`
	// NoPriceCountDetails: The reasons that contributed to the no price count (for
	// example, live pricing not available) and the total count for each reason.
	NoPriceCountDetails *NoPriceCountDetails `json:"noPriceCountDetails,omitempty"`
	// NoTaxBreakdownCount: The total number of missed participation due to one or
	// more of your hotels not specifying taxes and fees separately.
	...
	// PriceMissingCount: The total number of missed participations due to either a
	// price not being present in Google's cache or failing to successfully respond
	// to live pricing. Comprised of the following: * Bandwidth depleted * Cache
	// rate missing * Itinerary blocked * Live pricing not set up * Live pricing
	// timeout * Live pricing error
	PriceMissingCount int64 `json:"priceMissingCount,omitempty,string"`
	// PriceMissingCountDetails: The reasons that contributed to the price missing
	// count.
	PriceMissingCountDetails *PriceMissingCountDetails `json:"priceMissingCountDetails,omitempty"`
	// PriceProblemCount: The total number of missed participation due to an issue
	// with the accuracy of the price provided for the itinerary. Comprised of the
	// following: * Hotel suspended * Price unusually high * Price unusually low *
	...
	// PriceUnavailableCount: The total number of missed participation due to price
	// listed as unavailable (-1) for the requested itinerary. Comprised of the
	// following: * Price unavailable * Participation not likely * Other
	PriceUnavailableCount int64 `json:"priceUnavailableCount,omitempty,string"`
	// PriceUnavailableCountDetails: The reasons that contributed to the price
	// unavailable count.
...
// NoPriceCountDetails: The reasons that contributed to the no price count and
// the total count for each reason.
type NoPriceCountDetails struct {
	// LivePricingConfigIssueCount: The total number of missed participation due to
	// live pricing not being triggered for any of the following reasons: * You
	// didn't have live pricing configured for these searches. * You restricted
	// Google from accessing the hotel itinerary in question.
	LivePricingConfigIssueCount int64 `json:"livePricingConfigIssueCount,omitempty,string"`
	// LivePricingNotAvailableCount: The total number of missed participation due
	// to live pricing being unavailable. Live pricing will not be triggered for
	// certain default itineraries or UIs. In this scenario, partners will need a
	// cached price to participate.
	LivePricingNotAvailableCount int64 `json:"livePricingNotAvailableCount,omitempty,string"`
	// LivePricingNotTriggeredCount: The total number of missed participation due
	// to live pricing not being triggered for any of the following reasons: * You
	// didn't set a bid. * You didn't have a valid landing page. * There weren't
	// enough prices in the cache.
	LivePricingNotTriggeredCount int64 `json:"livePricingNotTriggeredCount,omitempty,string"`
	// LivePricingOtherReasonCount: The number of missed participations due to
	// other issues with live pricing.
	LivePricingOtherReasonCount int64 `json:"livePricingOtherReasonCount,omitempty,string"`
	// LivePricingTechnicalIssueCount: The total number of missed participation due
	// to technical issues with live pricing for any of the following reasons: *
	// You didn’t respond quickly enough and exceeded the response deadline
	// (around 4000 milliseconds). * You returned an error. * Your response was
	// malformed.
	LivePricingTechnicalIssueCount int64 `json:"livePricingTechnicalIssueCount,omitempty,string"`
	// ForceSendFields is a list of field names (e.g.
	// "LivePricingConfigIssueCount") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "LivePricingConfigIssueCount") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ParsedListing struct {
	// Brand: If not empty, it indicates that this listing belongs to a brand of
	// the feed.
	...
	// Review: Reviews associated with this listing. Each review has a single
...
// ParticipationResult: Represents a result from querying for participation
// stats for an account.
type ParticipationResult struct {
	// Key: Key of the result.
	Key *Key `json:"key,omitempty"`
	// MissedParticipationCount: The total number of opportunities **not** eligible
	// for the Google Ads auction process. Comprised of the following: * Landing
	// page missing * Price missing * Price problem * Price unavailable * Other
	MissedParticipationCount int64 `json:"missedParticipationCount,omitempty,string"`
	// MissedParticipationCountDetails: The reasons that contributed to the missed
	// participation count (for example, no availability) and a total count for
	// each reason.
	MissedParticipationCountDetails *MissedParticipationCountDetails `json:"missedParticipationCountDetails,omitempty"`
	// OpportunityCount: For a specific hotel, the total number of opportunities
	// that were available. Opportunities are the total number of instances when a
	// hotel ad could have been displayed to a user.
	OpportunityCount int64 `json:"opportunityCount,omitempty,string"`
	// ParticipationCount: The total number of opportunities for which you were
	// eligible to enter in the Google Ads auction process.
	ParticipationCount int64 `json:"participationCount,omitempty,string"`
	// ParticipationPercent: The percentage rate of participation where the number
	// of successfully participated opportunities is divided by the total number of
	// opportunities. For example, if a property was eligible to enter the Google
	// Ads auction 90 times out of 100 opportunities, the participation rate is
	// 90%.
	ParticipationPercent float64 `json:"participationPercent,omitempty"`
	// PartnerHotelDisplayName: Partner's hotel display name. This field is only
	// populated when the result is aggregated by `partnerHotelId`.
	PartnerHotelDisplayName string `json:"partnerHotelDisplayName,omitempty"`
...
type PriceAccuracyRow struct {
	...
	// AffectsScore: True if this row affects the overall price accuracy score.
	...
	// CheckinDate: Check-in date.
	CheckinDate *Date `json:"checkinDate,omitempty"`
	// ChildOccupancy: The number of children in the occupancy details of the
	// validation query.
	ChildOccupancy int64 `json:"childOccupancy,omitempty"`
	// CorrectionTime: Time at which an incorrect price is updated to a correct
	// price.
	...
	LengthOfStayDays int64 `json:"lengthOfStayDays,omitempty"`
	// MismatchReason: The reason why the fetched price didn't match the cached
	// price.
	//
	// Possible values:
	...
	// RateRuleId: The rate rule of the advertised price for non-public rates.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdultOccupancy") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PriceAccuracyView: A price accuracy view. Covers the price accuracy reports
// functionality in pre-v3.0 API versions. For more information, refer to Price
// Accuracy report (https://support.google.com/hotelprices/answer/6318506).
...
// PriceCoverageBucket: Coverage stats for one combination of advance booking
// window and length of stay.
type PriceCoverageBucket struct {
	...
	//   "ADVANCE_BOOKING_WINDOW_RANGE_UNKNOWN" - The value is unknown in this
	// version.
	//   "DAYS_0_TO_30" - Represents advance booking window between 0 and 30 days.
	//   "DAYS_31_TO_60" - Represents advance booking window between 31 and 60
	// days.
	//   "DAYS_61_TO_90" - Represents advance booking window between 61 and 90
	// days.
	//   "DAYS_91_TO_120" - Represents advance booking window between 91 and 120
	// days.
	//   "DAYS_121_TO_150" - Represents advance booking window between 121 and 150
	// days.
	//   "DAYS_151_TO_180" - Represents advance booking window between 151 and 180
	// days.
	//   "DAYS_181_TO_210" - Represents advance booking window between 181 and 210
	// days.
	//   "DAYS_211_TO_240" - Represents advance booking window between 211 and 240
	// days.
	//   "DAYS_241_TO_270" - Represents advance booking window between 241 and 270
	// days.
	//   "DAYS_271_TO_300" - Represents advance booking window between 271 and 300
	// days.
	//   "DAYS_301_TO_330" - Represents advance booking window between 301 and 330
	// days.
	AdvanceBookingWindowRange string `json:"advanceBookingWindowRange,omitempty"`
	// AvailablePriceCount: Number of prices for this advance booking window bucket
	// and length of stay bucket.
	...
	// PriceCoveragePercent: The percent of itineraries for which you have coverage
	// for this advance booking window bucket and length of stay bucket.
	PriceCoveragePercent float64 `json:"priceCoveragePercent,omitempty"`
	// ForceSendFields is a list of field names (e.g. "AdvanceBookingWindowRange")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdvanceBookingWindowRange") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PriceCoverageView: A price coverage view. Covers the `price_coverage_stats`
// Scorecard functionality in pre-v3.0 API versions. For more information,
// refer to Price Coverage for Push and Hint partners.
type PriceCoverageView struct {
	...
	// MatchedPropertyCount: The total number of properties that have prices for
	// the given itineraries.
	MatchedPropertyCount int64 `json:"matchedPropertyCount,omitempty"`
	// PriceCoverageBinaryPercent: The ratio between the number of hotels which
	// have at least one price for the calculation period and
	// `matched_property_count`.
	PriceCoverageBinaryPercent float64 `json:"priceCoverageBinaryPercent,omitempty"`
	// PriceCoverageBuckets: Price coverage stats for combinations of advance
	// booking window and length of stay ranges.
	PriceCoverageBuckets []*PriceCoverageBucket `json:"priceCoverageBuckets,omitempty"`
	// PriceCoveragePercent: The overall price coverage for an account. This value
	// is the ratio between the number of hotel prices for the calculation booking
	// window and length of stay range divided by the number of all possible
	// prices, which is `matched_property_count` times 330 (for advance book
	// window) times 30 (for length of stay).
	PriceCoveragePercent float64 `json:"priceCoveragePercent,omitempty"`
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CalculationDate") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type PriceMissingCountDetails struct {
	// BandwidthDepletedCount: No price was cached for this itinerary, and there
	// was no live query quota remaining.
	BandwidthDepletedCount int64 `json:"bandwidthDepletedCount,omitempty,string"`
	// CacheRateMissingCount: No price exists in the cache for this itinerary. A
	// live query was not done due to page constraints.
	CacheRateMissingCount int64 `json:"cacheRateMissingCount,omitempty,string"`
	// ItineraryBlockedCount: The itinerary was outside of your basic parameters,
	// so no price was pulled for the itinerary from either live query or cache
	// fill.
	ItineraryBlockedCount int64 `json:"itineraryBlockedCount,omitempty,string"`
	// LivePricingErrorCount: No price was cached for this itinerary. A live query
	// did not time out, but your system returned an error.
	LivePricingErrorCount int64 `json:"livePricingErrorCount,omitempty,string"`
	// LivePricingNotSetupCount: No price was cached for this itinerary, and live
	// query was not configured for this account.
	LivePricingNotSetupCount int64 `json:"livePricingNotSetupCount,omitempty,string"`
	// LivePricingTimeoutCount: No price was cached for this itinerary, and a live
	// query sent to your system timed out.
	LivePricingTimeoutCount int64 `json:"livePricingTimeoutCount,omitempty,string"`
	// ForceSendFields is a list of field names (e.g. "BandwidthDepletedCount") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BandwidthDepletedCount") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type PriceProblemCountDetails struct {
	...
	// PriceUnusuallyLowCount: The price given for this itinerary seemed oddly low
...
// PriceUnavailableCountDetails: The reasons that contributed to the price
// unavailable count and the total count for each reason.
type PriceUnavailableCountDetails struct {
	// ParticipationNotLikelyCount: No price was cached for this itinerary, and no
	// live query was done because your server usually tells us the hotel is
	// unavailable or sold out.
	ParticipationNotLikelyCount int64 `json:"participationNotLikelyCount,omitempty,string"`
	// PriceUnavailableCount: Hotel did not participate because it wasn't available
	// for the itinerary dates.
	PriceUnavailableCount int64 `json:"priceUnavailableCount,omitempty,string"`
	// ForceSendFields is a list of field names (e.g.
	// "ParticipationNotLikelyCount") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ParticipationNotLikelyCount") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PriceView: A price view. Covers the Prices functionality in pre-v3.0 API
// versions.
...
type PropertyPerformanceResult struct {
	// AdsClickCount: The total number of ad clicks that were recorded for this
	// result.
	...
	// AdsImpressionCount: The total number of ad impressions that were recorded
	// for this result.
	AdsImpressionCount int64 `json:"adsImpressionCount,omitempty,string"`
	// AdvanceBookingWindow: Difference in days between query date and check-in
	// date in property's local timezone. Only present if `advanceBookingWindow` is
	// specified in `aggregateBy` in the request.
	//
	// Possible values:
	//   "ADVANCE_BOOKING_WINDOW_UNSPECIFIED" - Not specified.
	//   "ADVANCE_BOOKING_WINDOW_SAME_DAY" - Check-in date was same as query date.
	//   "ADVANCE_BOOKING_WINDOW_NEXT_DAY" - Check-in date was next day after query
	// date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_2_TO_7" - Check-in date was 2-7 days after
	// query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_8_TO_14" - Check-in date was 8-14 days after
	// query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_15_TO_30" - Check-in date was 15-30 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_31_TO_60" - Check-in date was 31-60 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_61_TO_90" - Check-in date was 61-90 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_91_TO_120" - Check-in date was 91-120 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_121_TO_150" - Check-in date was 121-150 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_151_TO_180" - Check-in date was 151-180 days
	// after query date.
	//   "ADVANCE_BOOKING_WINDOW_DAYS_OVER_180" - Check-in date was over 180 days
	// after query date.
	AdvanceBookingWindow string `json:"advanceBookingWindow,omitempty"`
	// Brand: Partner-specified brand for the property. Only present if `brand` is
	// specified in `aggregateBy` in the request.
	Brand string `json:"brand,omitempty"`
	// ClickCount: The total number of free booking link clicks that were recorded
	// for this result.
	ClickCount int64 `json:"clickCount,omitempty,string"`
	// ClickthroughRate: Equal to `click_count` divided by `impression_count`.
	...
	//   "TABLET" - Tablets with full browsers.
	DeviceType string `json:"deviceType,omitempty"`
	// HighIntentUsers: Whether the user’s query indicated a strong interest in
	// booking. Only present if `highIntentUsers` is specified in `aggregateBy` in
	// the request.
	HighIntentUsers bool `json:"highIntentUsers,omitempty"`
	// ImpressionCount: The total number of free booking link impressions that were
	// recorded for this result. This value is rounded to preserve user privacy.
	...
	// Occupancy: Requested number of people staying at the property. Only present
	// if `partnerPropertyId` is specified in `aggregateBy` in the request.
	//
	// Possible values:
	...
	// `partnerPropertyDisplayName` is specified in `aggregateBy` in the request.
	...
	// property. Only present if `propertyRegionCode` is specified in `aggregateBy`
	// in the request
	PropertyRegionCode string `json:"propertyRegionCode,omitempty"`
	// UserRegionCode: ISO 3116 region code of the country/region of the user. Only
	// present if `userRegionCode` is specified in `aggregateBy` in the request
	UserRegionCode string `json:"userRegionCode,omitempty"`
	// VrWebsiteButtonClicks: The total number of clicks on the "Website" button on
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdsClickCount") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type Rating struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "RatingScale") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// ReconciliationReport: Reconciliation report (Hotel Ads only). Covers the
// Reconciliation Reports functionality in pre-v3.0 API versions. Lets you see
// the status, validate, and upload your Commissions reconciliation reports.
// This only applies if you are in the Google Hotel Ads Commissions Program
// (GHACP). For more information about working with reconciliation reports in
// Hotel Center, refer to Reconciliation reports
// (//support.google.com/hotelprices/answer/7019060#sending).
type ReconciliationReport struct {
	// Contents: Required. The contents of the commission report. Report contents
	// must conform to the requirements specified in [Reconciliation reports]
	...
	// Name: Resource name in the format
	// `accounts/{account_id}/reconciliationReports/{datetime}~{filename}`. The
	// value for `{datetime}` must be from 0001-01-01T00:00:00 to
...
// ReconciliationReportValidationIssue: Represents an issue encountered when
// validating a reconciliation report.
type ReconciliationReportValidationIssue struct {
	// Description: Text description of the issue, typically including what was
	// seen and why it was invalid.
	Description string `json:"description,omitempty"`
	// FieldName: Name of the invalid field. If no field name is given, this issue
	// applies to the whole line (or file).
	FieldName string `json:"fieldName,omitempty"`
	// LineNum: The line number on which the issue was detected. If this field is
	// 0, the issue applies to the whole file.
...
type Review struct {
	...
	// LanguageCode: Language of the review in ISO639 format, such as "en", "de",
	// etc.
	LanguageCode string `json:"languageCode,omitempty"`
	// Link: The url of the review.
	Link string `json:"link,omitempty"`
	// Rating: Any ratings associated with this review. This is repeated because
	// reviews can include ratings on different aspects of a listing, e.g.
	// location, cleanliness, etc.
	...
	Type string `json:"type,omitempty"`
	// VisitTime: Unix timestamp (in seconds) of when the stay was, in UTC+0.
...
type SetLiveOnGoogleRequest struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "LiveOnGoogle") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SetLiveOnGoogleResponse struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "FailedHotelIds") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SummarizeHotelViewsResponse struct {
	...
	// OverclusteredPropertyWithErrorsCount: The number of overclustered properties
	...
	// UnmatchedPropertyWithErrorsCount: The number of unmatched properties that
	// have data-related errors.
	UnmatchedPropertyWithErrorsCount int64 `json:"unmatchedPropertyWithErrorsCount,omitempty,string"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "LastFeedSubmissionTime") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "LastFeedSubmissionTime") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SummarizePriceAccuracyResponse struct {
	// CurrentBookOnGoogleScore: The current Book on Google price accuracy score.
	//
	// Possible values:
	...
	// CurrentOverallScore: The current price accuracy score combining both website
	// and Book on Google scores.
	//
	// Possible values:
	//   "PRICE_ACCURACY_STATE_UNSPECIFIED" - Not specified.
	//   "PRICE_ACCURACY_STATE_UNKNOWN" - The value is unknown in this version.
	...
	// CurrentScore: The current price accuracy score. Contains the same value as
	// `current_website_score`. For clarity, use either `current_website_score` or
	// 'current_overall_score` depending upon which is needed.
	//
	// Possible values:
	//   "PRICE_ACCURACY_STATE_UNSPECIFIED" - Not specified.
	//   "PRICE_ACCURACY_STATE_UNKNOWN" - The value is unknown in this version.
	...
	// PredictedBookOnGoogleScore: The predicted Book on Google price accuracy
	// score.
	//
	// Possible values:
	//   "PRICE_ACCURACY_STATE_UNSPECIFIED" - Not specified.
	...
	// PredictedOverallScore: The predicted price accuracy score combining both
	...
	// PredictedScore: The predicted price accuracy score. Contains the same value
	// as `predicted_website_score`. For clarity, use either
	// `predicted_website_score` or 'predicted_overall_score` depending upon which
	// is needed.
	//
	// Possible values:
	//   "PRICE_ACCURACY_STATE_UNSPECIFIED" - Not specified.
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "CurrentBookOnGoogleScore")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CurrentBookOnGoogleScore") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type VerifyListingsRequest struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "XmlListing") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type VerifyListingsResponse struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ParsedListing") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Create: Creates a new account link between a Hotel Center account and a
// Google Ads account.
//
//   - parent: The resource name of the Hotel Center account being queried. The
//     format is `accounts/{account_id}`.
...
// *AccountLink.ServerResponse.Header or (if a response was returned at all) in
...
//
//   - name: The resource name of the account link being deleted. The format is
//     `accounts/{account_id}/accountLinks/{account_link_id}`.
...
// Get: Returns the `AccountLink` instance associated with the account link
// resource name specified in the request URL. To get account link resource
// names, use the `list` method.
//
...
// UpdateMask sets the optional parameter "updateMask": Required. The field to
// update. Only the `accountLinkTarget` field can be updated. Use the syntax
// shown in the example URL below and provide the new value in the request
// body. Example request URL and request body: ``` PATCH
...
// { "account_link_target": { "hotel_list": { "partner_hotel_ids": ["1234",
// "4321"] } } } ``` The response body will only include the `name` field of an
// `AccountLink`. The information above is sufficient for forming the URL and
// request body. The sentence below is auto-generated, supplemental information
...
// existing properties, you only need this operation when you want to configure
// a brand before you send its properties to Google. Note that it might take a
// couple of days after your listing feed first provides a brand for the brand
// to appear.
//
//   - parent: The resource name of the Hotel Center account being queried. The
//     format is `accounts/{account_id}`.
...
// Get: Returns the `Brand` instance associated with the request URI's brand
// resource name.
//
...
//
//   - name: Output only. The resource name for the brand in the format
//     `accounts/{account_id}/brands/{brand_id}`. The `brand_id` corresponds to
//     the partner's brand identifier used for landing page matching and the
//     property-level brand identifier. A default brand is applied to properties
//     that do not have a brand. The `brand_id` of the default brand is
//     `NO_BRAND_ID`. It can be fetched and updated like any configured brand.
...
// AllowMissing sets the optional parameter "allowMissing": When true, and the
// Brand is not found, a new Brand will be created. In this situation,
...
// UpdateMask sets the optional parameter "updateMask": Required. The field to
// update. Only the `display_names` and `icon` fields can be updated. Use the
// syntax shown in the example URI below and provide the new value in the
// request body. Example request URI and request body: ``` PATCH
// https://travelpartner.googleapis.com/v3/accounts/123456789/
// brands/my-brand?update_mask=brand.display_names ``` ``` { "display_names":
// [{ "language": "en" "text": "Gilles' Gites" }] } ``` The information above
// is sufficient for forming the URI and request body. The sentence below is
// auto-generated, supplemental information about the `FieldMask` format in
// general.
...
// PropertyPerformanceReportService.QueryPropertyPerformanceReport, which also
// has impression reporting, instead. Provides the ability to query (get,
// filter, and segment) a free booking links report for a specific account.
//
...
// AggregateBy sets the optional parameter "aggregateBy": Specifies how to
// segment the metrics returned by the query. For example, if `userRegionCode`
// is specified, the `freeBookingLinksResult` will provide metrics aggregated
// by user region. The string value is a comma-separated list of fields. Valid
// fields are: `date`, `userRegionCode`, `deviceType`, `partnerHotelId`, and
// `partnerHotelDisplayName`. Only fields specified here are included in the
...
// Filter sets the optional parameter "filter": The conditions (fields and
// expressions) used to filter the free booking link metrics for the account
// being queried. The syntax requires spaces surrounding the `in` operator.
// Otherwise, spaces can be omitted. Conditions can be joined using the `and`
// operator. The `date` field is required. All other fields are optional. The
// `date` field values are inclusive and must be in YYYY-MM-DD format. The
// earliest acceptable date is 2021-03-09; earlier date values will be coerced
// to 2021-03-09. Values for `partnerHotelDisplayName` are matched
// case-insensitively. Examples of valid conditions are as follows: * `date =
// '2021-12-03'` * `date between '2021-12-03' and '2021-12-08'` * `deviceType =
// 'TABLET'` * `deviceType in ('MOBILE', 'TABLET')` * `partnerHotelId = 'AAA'`
// * `partnerHotelId in ('AAA', 'BBB')` * `partnerHotelDisplayName = 'hotel A'`
// * `partnerHotelDisplayName in ('Hotel A', 'HOTEL b')` * `userRegionCode =
// 'US'` * `userRegionCode in ('US', 'CA')`
...
// participation results to return. The service may return fewer than this
// value. If unspecified, at most 10,000 results will be returned. The maximum
// value is 10,000; values above 10,000 will be coerced to 10,000.
...
// from a previous QueryParticipationReport request. Provide this to receive
// the subsequent page. When paginating, all other parameters provided to
// QueryParticipationReport must match the call that provided the page token.
...
// Any non-2xx status code is an error. Response headers are in either
// *QueryFreeBookingLinksReportResponse.ServerResponse.Header or (if a response
...
// Filter sets the optional parameter "filter": The conditions (fields and
// expressions) used to filter HotelViews. The syntax requires spaces
...
// PageSize sets the optional parameter "pageSize": Number of elements to
// retrieve in a single page.
func (c *AccountsHotelViewsListCall) PageSize(pageSize int64) *AccountsHotelViewsListCall {
	c.urlParams_.Set("pageSize", fmt.Sprint(pageSize))
	return c
}

// PageToken sets the optional parameter "pageToken": Token of the page to
// retrieve.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListHotelViewsResponse.ServerResponse.Header or (if a response was returned
...
// Any non-2xx status code is an error. Response headers are in either
// *SummarizeHotelViewsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// SetLiveOnGoogle: Collection-level custom method to update the Live on Google
// status for multiple properties. Each call can turn on or off multiple
// hotels. To turn some hotels on and turn some hotels off, you will have to
// make multiple calls.
//
...
// Create: Uploads a new icon and starts its review process. Generates an
// `icon_id` and includes it in the icon's resource name, which is the format
// `accounts/{account_id}/icons/{icon_id}` Returns HTTP status 400 and doesn't
// trigger the review process if the icon has any of these conditions: * Image
// is not in PNG format, or not convertible to PNG format. * Size less than 72
// pixels * Size greater than 1200 pixels * Aspect ratio other than 1:1
//
//   - parent: The resource name of the partner account owning the icon. The
//     format is `accounts/{account_id}`.
...
// Get: Returns the `Icon` instance associated with an icon resource name
// specified in the request URI. To get icon resource names, use the `list`
// method.
//
...
//
//   - parent: The resource name of the queried partner account. The format is
//     `accounts/{account_id}`.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListIconsResponse.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *VerifyListingsResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// AggregateBy sets the optional parameter "aggregateBy": Specifies how to
// segment the metrics returned by the query. For example, if `userRegionCode`
// is specified as the `aggregate_by` value, the `participationResult` will
// provide metrics aggregated by user region. The string value is a
// comma-separated list of fields. Valid fields are: `date`, `userRegionCode`,
// `deviceType`, `partnerHotelId`, `hotelRegionCode`, `advanceBookingWindow`,
// `lengthOfStayDays`, `checkinDate`, and `occupancy`. Fields that are not
// specified are not included in the ParticipationResult. **Important**: Using
// the `aggregateBy` parameter with `partnerHotelId` or more than two values
// can result in an error. Filtering with `partnerHotelId` and `date` to reduce
// the number of rows can prevent the occurrence of errors.
...
// Filter sets the optional parameter "filter": The conditions (fields and
// expressions) used to filter the participation metrics for the account being
// queried. The syntax requires spaces surrounding the `in` operator.
// Otherwise, spaces can be omitted. Conditions can be joined using the `AND`
// operator. **Important:** The correct syntax to add multiple conditions is:
...
// Any non-2xx status code is an error. Response headers are in either
// *QueryParticipationReportResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
//
// - name: The resource name of price accuracy view that is being queried.
...
// IncludeMatchedPrices sets the optional parameter "includeMatchedPrices": Set
// to true if matched prices are to be added into the report.
...
// IncludeNonScoring sets the optional parameter "includeNonScoring": Set to
// true if non-scoring rows are to be added into the report.
...
// IncludePixels sets the optional parameter "includePixels": Set to true if
// pixel signals are to be added into the report.
...
// Any non-2xx status code is an error. Response headers are in either
// *PriceAccuracyView.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListPriceAccuracyViewsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
//
//   - parent: The resource name of the account that is being queried. The format
...
// Any non-2xx status code is an error. Response headers are in either
// *SummarizePriceAccuracyResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *PriceCoverageView.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListPriceCoverageViewsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *PriceView.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
...
// Query: Provides the ability to query (get, filter, and segment) a property
// performance links report for a specific account.
//
//   - name: The resource name of the account being queried. Format:
//     accounts/{account_id}.
...
// AggregateBy sets the optional parameter "aggregateBy": Specifies how to
// segment the metrics returned by the query. For example, if `userRegionCode`
// is specified, the `PropertyPerformanceResult` will provide metrics
// aggregated by user region. The string value is a comma-separated list of
// fields. Valid fields are: `advanceBookingWindow`, `brand`, `date`,
// `deviceType`, `highIntentUsers`, `lengthOfStay`, `propertyRegionCode`,
// `occupancy`, `partnerPropertyId`, `partnerPropertyDisplayName`, and
// `userRegionCode`. Only fields specified here are included in the
// PropertyPerformanceResult. **Important**: Using the `aggregateBy` parameter
// with `partnerHotelId` or more than two values can result in an error.
// Filtering with `partnerHotelId` and `date` to reduce the number of rows can
...
// Filter sets the optional parameter "filter": The conditions (fields and
// expressions) used to filter the property performance metrics for the account
// being queried. The syntax requires spaces surrounding the `in` operator.
// Otherwise, spaces can be omitted. Conditions can be joined using the `AND`
// operator. **Important:** The correct syntax to add multiple conditions is:
// "?filter==%20AND%20=..." The `date` field is required. All other fields
// are optional. The `date` field values are inclusive and must be in
// YYYY-MM-DD format. The earliest acceptable date is 2021-03-09; earlier date
// values will be coerced to 2021-03-09. Values for
// `partnerPropertyDisplayName` and `brand` are matched case-insensitively.
// Examples of valid conditions are as follows: * `advanceBookingWindow =
// 'ADVANCE_BOOKING_WINDOW_SAME_DAY'` * `advanceBookingWindow in
// ('ADVANCE_BOOKING_WINDOW_SAME_DAY', 'ADVANCE_BOOKING_WINDOW_DAYS_61_TO_90')`
// * `brand = 'Brand A'` * `brand in ('Brand A', 'brand B')` * `date =
// '2021-12-03'` * `date between '2021-12-03' and '2021-12-08'` * `deviceType =
// 'TABLET'` * `deviceType in ('MOBILE', 'TABLET')` * `highIntentUsers =
// 'TRUE'` * `highIntentUsers = 'FALSE'` * `lengthOfStay =
// 'LENGTH_OF_STAY_NIGHTS_2'` * `lengthOfStay in ('LENGTH_OF_STAY_NIGHTS_2',
// 'LENGTH_OF_STAY_NIGHTS_4_TO_7')` * `propertyRegionCode = 'US'` *
// `propertyRegionCode in ('US', 'CA')` * `occupancy = 'OCCUPANCY_2'` *
// `occupancy in ('OCCUPANCY_2', 'OCCUPANCY_OVER_4')` * `partnerPropertyId =
// 'AAA'` * `partnerPropertyId in ('AAA', 'BBB')` * `partnerPropertyDisplayName
...
// Any non-2xx status code is an error. Response headers are in either
// *QueryPropertyPerformanceReportResponse.ServerResponse.Header or (if a
...
// Any non-2xx status code is an error. Response headers are in either
// *CreateReconciliationReportResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
//
//   - name: The resource name of the reconciliation report to fetch. The format
//     is `accounts/{account_id}/reconciliationReports/{datetime}~{filename}`.
...
// Any non-2xx status code is an error. Response headers are in either
// *ReconciliationReport.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// EndDate sets the optional parameter "endDate": End of date range to fetch
// files for. Format is yyyy-mm-dd[THH[:MM[:SS]]]. If empty, reports until the
// end of time are fetched.
...
// StartDate sets the optional parameter "startDate": Beginning of date range
// to fetch files for. Format is yyyy-MM-dd[THH[:mm[:SS]]]. If empty, reports
// from the beginning of time onwards are fetched.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListReconciliationReportsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ValidateReconciliationReportResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
